...
export const useGetCell = <T extends string>(cellNameArray: T[], options?: { [key in T]?: { className?: string } }) => {
    ...
    const cellData = cellNameArray.map(name =>  recursiveMapSearch(columns, name, 'subColumns'))
    // const cellData = recursiveMapSearch(columns, )
    ...
    const cells = cellData.reduce((accum: TAccum, item) => {
        if(!item){
            debugger
            throw new Error(`not exist item: ${item}, ${cellData}`)
        }
